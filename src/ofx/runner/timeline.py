"""Command timeline logger for OFX step executions.

Writes entries to the oops-logger engagement CSV at
``$OOPS_LOG_DIR/$OOPS_ENGAGEMENT.csv`` with a fallback to
``~/.oops/logs/default.csv``. When a project is active, the engagement name
defaults to the project name so OFX workflow commands appear alongside shell
commands in the same engagement timeline.
...
def _csv_row(values: list[str]) -> str:
    """Produce a single CSV row string with proper quoting and escaping."""
    buf = io.StringIO()
...
def _ensure_csv(path: Path) -> None:
    """Create the CSV file with the oops-logger header if it does not exist."""
...
def log_step(
    *,
    ...
    try:
        ...
        logger.debug("Failed to write oops timeline entry: %s", exc)


__all__ = ["detect_target", "log_step"]
