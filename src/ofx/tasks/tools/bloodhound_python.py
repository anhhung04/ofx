...
class BloodhoundPythonTask(Task):
    ...
    opts = {
        ...
        "collection": OptDef(flag="-c", type=str, help="Collection methods (All,Group,Session,Trusts,ACL,...)"),
        "output_dir": OptDef(flag="--zip", is_flag=True, help="Compress output into zip"),
        "dns_tcp": OptDef(flag="--dns-tcp", is_flag=True, help="Use TCP for DNS"),
        "dns_timeout": OptDef(flag="--dns-timeout", type=int, help="DNS timeout in seconds"),
        "use_ldaps": OptDef(flag="--use-ldaps", is_flag=True, help="Use LDAP over SSL"),
        "auth_method": OptDef(flag="-a", type=str, help="Auth method (auto/ntlm/kerberos)"),
    }
    ...
    def parse_output(
        ...
        for line in raw.splitlines():
            ...
            if "Done in" in line:
                results.append(Tag(name="status", value="completed", category="bloodhound"))
            # INFO: Found 42 users
            elif "Found" in line:
                results.append(Tag(name="collection_info", value=line.split("INFO:")[-1].strip() if "INFO:" in line else line, category="bloodhound"))
            # Output file names
            elif line.endswith(".json") or line.endswith(".zip"):
                results.append(Tag(name="output_file", value=line, category="bloodhound"))
