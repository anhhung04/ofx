...
class VisualizeHandler:
    ...
    def _extract_step_info(self, steps) -> list[str]:
        ...
        for step in steps:
            if step.name:
                ...
                step_list.append(
                    cmd[:max_cmd_length] + "..." if len(cmd) > max_cmd_length else cmd
                )
            elif step.script:
    ...
    def _draw_simple_graph(self, job_deps: dict, job_details: dict) -> None:
        ...
        deps_relationships = [
            (dep, job_id) for job_id, deps in job_deps.items() if deps for dep in deps
        ]
        ...
        for stage_idx, stage_jobs in enumerate(stages):
            ...
            job_boxes, job_deps_lines = self._prepare_stage_boxes(
                stage_jobs, job_details, job_deps, box_width
            )
            ...
            self._print_stage_dependencies(
                job_deps_lines, stage_jobs, box_width, box_spacing
            )
    ...
    def _prepare_stage_boxes(
        self, stage_jobs: list, job_details: dict, job_deps: dict, box_width: int
    ) -> tuple:
        ...
        for job_id in stage_jobs:
            box_lines = self._create_simple_job_box(
                job_id, job_details[job_id], box_width
            )
    ...
    def _pad_boxes_to_height(
        self, job_boxes: list, target_height: int, box_width: int
    ) -> None:
    ...
    def _print_stage_dependencies(
        self, job_deps_lines: list, stage_jobs: list, box_width: int, box_spacing: int
    ) -> None:
        ...
        if len(stage_jobs) == 1:
            ...
            if any(job_deps_lines):
                ...
                for idx, dep_text in enumerate(job_deps_lines):
                    if dep_text:
                        current_pos = len(
                            dep_line.replace("[dim]", "").replace("[/dim]", "")
                        )
                        target_pos = (
                            base_indent
                            + idx * (box_width + border_width + box_spacing)
                            + sub_indent
                        )
    ...
    def _create_simple_job_box(
        self, job_id: str, job_detail: dict, width: int
    ) -> list[str]:
        ...
        if step_list:
            ...
            steps_line = (
                f"  {job_detail['steps']} step{'s' if job_detail['steps'] != 1 else ''}"
            )
    ...
    def _find_parallel_schedule(self, job_keys: list, deps_relationships: list) -> list:
        ...
        if not stages or not all_jobs_processed:
            return self._calculate_dependency_levels_as_stages(
                job_keys, deps_relationships
            )

        return stages

    def _calculate_dependency_levels_as_stages(
        self, job_keys: list, deps_relationships: list
    ) -> list:
        ...
        for level in range(max_level + 1):
            stage_jobs = [
                job for job, job_level in levels.items() if job_level == level
            ]
            if stage_jobs:
    ...
    def _render_ascii_flowchart(
        self, execution_stages: list, job_deps: dict, job_details: dict
    ) -> Text:
        ...
        for stage_idx, stage_jobs in enumerate(execution_stages):
            ...
            job_box_width = min(
                35, (max_width - (len(stage_jobs) - 1) * 3) // len(stage_jobs)
            )
            ...
            if len(stage_jobs) == 1:
                ...
                boxes = [
                    self._create_job_box(jid, job_details[jid], job_box_width)
    ...
    def _draw_stage_connectors(
        ...
        if not has_explicit_deps:
            if from_count == 1 and to_count == 1:
                ...
                line_end = (
                    to_count * box_width + (to_count - 1) * spacing - (box_width // 2)
                )
                ...
                for i in range(to_count):
                    ...
                    arrow_line += (
                        " " * (pos - len(arrow_line) + len(base_indent))
                        + f"[yellow]{ARROW_DOWN}[/yellow]"
                    )
                ...
                for i in range(from_count):
                    ...
                    arrow_line += (
                        " " * (pos - len(arrow_line) + len(base_indent))
                        + f"[dim yellow]{V_LINE}[/dim yellow]"
                    )
                connector_lines.append(arrow_line)

                line_start = box_width // 2
                line_end = (
                    from_count * box_width
                    + (from_count - 1) * spacing
                    - (box_width // 2)
                )
                line = base_indent + " " * line_start
                for i in range(line_end - line_start + 1):
                ...
                for i in range(from_count):
                    pos = i * (box_width + spacing) + box_width // 2
                    arrow_line2 += (
                        " " * (pos - len(arrow_line2) + len(base_indent))
                        + f"[yellow]{ARROW_DOWN}[/yellow]"
                    )
            ...
            for from_idx, from_job in enumerate(from_jobs):
                if deps_map[from_job]:
                    ...
                    line1 += (
                        " " * (from_center - len(line1) + len(base_indent))
                        + f"[dim cyan]{V_LINE}[/dim cyan]"
                    )
            ...
            for to_idx, to_job in enumerate(to_jobs):
                to_center = to_idx * (box_width + spacing) + box_width // 2
                has_dep = any(
                    to_job in deps_map.get(from_job, []) for from_job in from_jobs
                )
                if has_dep:
                    arrow_line += (
                        " " * (to_center - len(arrow_line) + len(base_indent))
                        + f"[cyan]{ARROW_DOWN}[/cyan]"
                    )
    ...
    def _create_job_box(self, job_id: str, job_detail: dict, width: int) -> list[str]:
        ...
        lines.append(
            f"[{color}]{V_LINE}[/{color}][bold {color}]{job_id_text}{' ' * padding}[/bold {color}][{color}]{V_LINE}[/{color}]"
        )
        ...
        lines.append(
            f"[{color}]{V_LINE}[/{color}][dim]{job_name}{' ' * padding}[/dim][{color}]{V_LINE}[/{color}]"
        )

        lines.append(
            f"[{color}]{V_LINE}[dim]{H_LINE * inner_width}[/dim]{V_LINE}[/{color}]"
        )
        ...
        lines.append(
            f"[{color}]{V_LINE}[/{color}][dim]{steps_text}{' ' * padding}[/dim][{color}]{V_LINE}[/{color}]"
        )
        ...
        if job_detail["has_outputs"] or job_detail["has_hooks"]:
            ...
            lines.append(
                f"[{color}]{V_LINE}[/{color}][dim italic]{features_text}{' ' * padding}[/dim italic][{color}]{V_LINE}[/{color}]"
            )

        lines.append(
            f"[{color}]{BOTTOM_LEFT}{H_LINE * inner_width}{BOTTOM_RIGHT}[/{color}]"
        )

        return lines

    def _draw_ascii_graph(
        self, level_jobs: dict, job_deps: dict, job_details: dict
    ) -> list:
    ...
    def _display_enhanced_job_table(self, job_details: dict) -> None:
        ...
        for job_id, details in job_details.items():
            ...
            complexity_label = (
                "Low" if complexity <= 3 else "Med" if complexity <= 6 else "High"
            )
            complexity_color = (
                "green" if complexity <= 3 else "yellow" if complexity <= 6 else "red"
            )
        ...
        summary.append(
            f"Jobs with outputs: {jobs_with_outputs}/{len(job_details)} | ", style="dim"
        )
        summary.append(
            f"Jobs with hooks: {jobs_with_hooks}/{len(job_details)}", style="dim"
        )
    ...
    def _display_job_steps(self, workflow) -> None:
        ...
        workflow_tree = Tree(
            f"[>] [bold red]{workflow.name}[/bold red]", guide_style="red"
        )

        for job_id, job in workflow.jobs.items():
            job_node = workflow_tree.add(
                f"[*] [bold yellow]{job_id}[/bold yellow] - {job.name or job_id}"
            )
            job_node.add(
                f"[dim]{len(job.steps)} step{'s' if len(job.steps) != 1 else ''}[/dim]"
            )
    ...
    def _add_steps_to_tree(
        self, parent_node, steps: list, depth: int = 0, max_depth: int = 3
    ) -> None:
        ...
        for i, step in enumerate(steps, 1):
            ...
            if step.uses:
                step_node = parent_node.add(
                    f"{step_prefix}: [bold green]{step_name}[/bold green] [dim][WORKFLOW][/dim]"
                )
                try:
                    workflow_dirs = [
                    ...
                    for sub_job_id, sub_job in subworkflow.jobs.items():
                        sub_job_node = subworkflow_node.add(
                            f"[*] [bold yellow]{sub_job_id}[/bold yellow] - {sub_job.name or sub_job_id}"
                        )
                        sub_job_node.add(
                            f"[dim]{len(sub_job.steps)} step{'s' if len(sub_job.steps) != 1 else ''}[/dim]"
                        )
                        self._add_steps_to_tree(
                            sub_job_node, sub_job.steps, depth + 1, max_depth
                        )
                ...
                step_node = parent_node.add(
                    f"{step_prefix}: [bold blue]{step_name}[/bold blue] [dim][CMD][/dim]"
                )
                ...
                step_node = parent_node.add(
                    f"{step_prefix}: [bold magenta]{step_name}[/bold magenta] [dim][SCRIPT][/dim]"
                )
                ...
                step_node = parent_node.add(
                    f"{step_prefix}: [bold red]{step_name}[/bold red] [dim][UNKNOWN][/dim]"
                )
    ...
    def _generate_plantuml(self, workflow: dict) -> str:
        ...
        if "jobs" in workflow:
            for job_name, job_config in workflow["jobs"].items():
                ...
                steps_count = (
                    len(job_config.get("steps", []))
                    if isinstance(job_config, dict)
                    else 0
                )
                lines.append(
                    f'rectangle "{job_name}\n{safe_description}\n({steps_count} steps)" as {job_name}'
                )
