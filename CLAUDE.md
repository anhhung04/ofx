...
- `Step` — exactly one run type must be present: `run` (shell command), `script` (inline Python), `script_file`, `uses` (reusable workflow), `task` (pre-built security tool wrapper), or `pipe` (declarative ETL pipeline). Aliases used extensively (e.g. `if`, `continue-on-error`, `retry-delay`).
- `PipeConfig` (`models/pipe.py`) — declarative ETL pipeline configuration with 14 operations: input (Jinja2 expression), filter/map (Python expressions with safe eval), flatten, sort, reverse, unique, group_by, offset, limit, format (json/csv/lines/jsonl/table), field, separator, headers.
...
- `execution/pipe.py` — `PipeRunner` for declarative ETL pipelines (filter, map, sort, unique, group_by, etc.)
...
- `tools/` — 26 tool wrappers: nmap, naabu, httpx, ffuf, feroxbuster, katana, subfinder, dnsx, nuclei, wafw00f, gospider, gau, dalfox, dirsearch, arjun, gitleaks, trufflehog, grype, trivy, wpscan, sshaudit, testssl, searchsploit, maigret, h8mail, whois. The 12 JSONL tools support live streaming via `parse_line()`.
- `TaskRunner` (`src/ofx/runner/tasks/runner.py`) — `BaseRunner[TaskExecution]` that builds the command, executes via `CommandExecutor`, parses output into typed objects, deduplicates via `_uuid`, and stores `typed_outputs` alongside regular outputs in the registry. Supports **live streaming** — when `task.supports_streaming` is True, uses `execute_streaming()` to parse output line-by-line and publishes items to channels in real-time. Supports **credential auto-storage** — when `store_creds` is enabled, `UserAccount` outputs are persisted to the exegol-history KeePass DB via `_store_credentials()`.
...
- **Credential storage**: `TaskRunner._store_credentials()` and `CloudStepRunner._store_credentials()` persist `UserAccount` typed outputs to `~/.exh/DB.kdbx` (exegol-history KeePass). Duplicates (same user+pass+hash+domain) are skipped. Gracefully degrades when pykeepass or DB is unavailable.
...
- **Credential auto-storage** (`store-creds`): Task steps producing `UserAccount` outputs can auto-store them in the credential DB. Control via step `store-creds: true/false`, workflow `defaults: { store-creds: true }`, or global `auto_store_creds` setting. Precedence: step > defaults > global. Works in both local and cloud runners.
- **Pipe steps**: Use `pipe:` for declarative ETL between steps. The `input` field is a Jinja2 expression (typically `{{ steps['name'].outputs.typed_outputs }}`). Operations apply in order: filter → map → flatten → sort → reverse → unique → group_by → offset → limit → format. Pipe steps are local-only (not supported in cloud runners). Results available as `outputs.stdout` (formatted text), `outputs.items` (processed list), and `outputs.file` (temp file path). ETL helpers (`pluck`, `sort_by`, `unique_by`, `where`, `to_lines`, `to_csv`, `to_jsonl`, etc.) are also available as Jinja2 filters in any template expression.
- **Output capture** (`add_outputs()`): In Python `script:` steps, use `add_outputs(key=val, ...)` to write step outputs (replaces manual `OFX_OUTPUTS` file writes). Dicts/lists are JSON-serialized. In `run:` steps, use `echo "key=val" >> $OFX_OUTPUTS`. Access via `{{ steps.N.outputs.key }}`.
- **Display truncation** (`max_display_lines`): Console output is capped at 50 lines by default (full output saved to log files). Set via `OFX_MAX_DISPLAY_LINES` env var or `max_display_lines` in `~/.ofx/config.yml`. Applied in both `StepRunner._log_output()` and `CloudStepRunner._log_output()`.
