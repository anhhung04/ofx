...
- **Executors** ([src/ofx/runner/executors/](../src/ofx/runner/executors/)): Concrete runner implementations
...
**Import convention**: Use public API `from ofx.runner import WorkflowRunner, RunContext, ...` or explicit paths `from ofx.runner.core import BaseRunner`.
...
- Tools block: `workflow.tools` installs binaries into `~/Tools/bin` via `ToolInstallerRunner`; can define `install`, optional `check`, and `post_install` commands ([src/ofx/runner/executors/tool_installer.py](../src/ofx/runner/executors/tool_installer.py)).
- Hooks: steps support `before_step`, `after_step`, `on_retry`, `on_skip`, and `on_timeout` scripts executed with the configured shell; workflow/job hooks exist in models but only step hooks have concrete runner support ([src/ofx/runner/lifecycle/hooks.py](../src/ofx/runner/lifecycle/hooks.py)).
- Control flow: `run_if` on jobs/steps must resolve truthy or the runner marks them canceled; job dependencies are rechecked at runtime and block execution if unmet ([src/ofx/runner/executors/job.py](../src/ofx/runner/executors/job.py)).
- Outputs: stdout logged and optionally persisted to timestamped files under the run `output_path` when `log_stdout` is truthy ([src/ofx/runner/executors/step.py](../src/ofx/runner/executors/step.py)). Empty output directories are removed after workflow completion.
